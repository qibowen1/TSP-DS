...
TSPDSSolution PerturbationOperators::perturbSolution3(TSPDSSolution& solution, int k) {
    ...
    if (params.route_perturbation_enabled) {
        doubleBridge(s);
    }
...
void PerturbationOperators::doubleBridge(TSPDSSolution& s)
{
    auto& r = s.truck_route;
    ...
    movable.reserve(n);
    for (int i = 1; i + 1 < n; ++i) {
        if (r[i] != graph.drone_station) movable.push_back(r[i]);
    }
    if (movable.size() < 8) return;

    std::shuffle(movable.begin(), movable.end(), gen);
    std::array<int, 4> cut{};
    std::uniform_int_distribution<int> dist(1, static_cast<int>(movable.size()) - 1);
    for (int guard = 0; guard < 100; ++guard) {
        for (int& x : cut) x = dist(gen);
        std::sort(cut.begin(), cut.end());
        if (cut[0] > 0 && cut[0] < cut[1] && cut[1] < cut[2] && cut[2] < cut[3] && cut[3] < static_cast<int>(movable.size())) break;
    }
    if (!(cut[0] < cut[1] && cut[1] < cut[2] && cut[2] < cut[3])) return;

    std::vector<int> a(movable.begin(), movable.begin() + cut[0]);
    std::vector<int> b(movable.begin() + cut[0], movable.begin() + cut[1]);
    std::vector<int> c(movable.begin() + cut[1], movable.begin() + cut[2]);
    std::vector<int> d(movable.begin() + cut[2], movable.begin() + cut[3]);
    std::vector<int> e(movable.begin() + cut[3], movable.end());

    std::vector<int> rebuilt;
    rebuilt.reserve(r.size());
    rebuilt.push_back(graph.depot);
    rebuilt.push_back(graph.drone_station);
    rebuilt.insert(rebuilt.end(), a.begin(), a.end());
    rebuilt.insert(rebuilt.end(), c.begin(), c.end());
    rebuilt.insert(rebuilt.end(), b.begin(), b.end());
    rebuilt.insert(rebuilt.end(), d.begin(), d.end());
