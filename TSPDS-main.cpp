...
static std::vector<int> getEnvIntList(const char* name, const std::vector<int>& fallback) {
    std::string s = getEnvString(name);
    if (s.empty()) return fallback;
    std::vector<int> out;
...
static std::vector<std::string> getEnvStringList(const char* name, const std::vector<std::string>& fallback) {
    std::string s = getEnvString(name);
    if (s.empty()) return fallback;
    std::vector<std::string> out;
    std::stringstream ss(s);
...
struct FinalValidationResult {
    ...
    double drone_active_time = 0.0;
    double drone_complete_time = 0.0;
    double makespan = 0.0;
    std::string errors;
};

static void addValidationError(FinalValidationResult& res, const std::string& msg) {
    if (!res.errors.empty()) res.errors += "; ";
    res.errors += msg;
    res.valid = false;
}

static FinalValidationResult validateFinalSolutionStrict(const TSPDSGraph& graph,
    const TSPDSSolution& solution) {
    FinalValidationResult res;
    const int n = static_cast<int>(graph.nodes.size());
    if (n <= 0) {
        addValidationError(res, "empty graph");
        return res;
    }
    if (solution.truck_route.size() < 2) {
        addValidationError(res, "truck route too short");
        return res;
    }
    if (solution.truck_route.front() != graph.depot || solution.truck_route.back() != graph.depot) {
        addValidationError(res, "truck route must start/end at depot");
    }
    ...
    for (int i = 0; i < static_cast<int>(solution.truck_route.size()); ++i) {
        ...
        if (node < 0 || node >= n) {
            addValidationError(res, "truck route contains invalid node " + std::to_string(node));
            continue;
        }
        if (node == graph.drone_station) {
            if (station_pos >= 0) addValidationError(res, "drone station appears more than once in truck route");
            station_pos = i;
        }
        if (i > 0 && i + 1 < static_cast<int>(solution.truck_route.size())) {
            served[node]++;
        }
        if (i + 1 < static_cast<int>(solution.truck_route.size())) {
            int to = solution.truck_route[i + 1];
            if (to >= 0 && to < n) res.truck_time += graph.truck_time[node][to];
        }
    }
    if (station_pos < 0) {
        addValidationError(res, "truck route does not visit drone station");
    }
    else {
        for (int i = 0; i < station_pos; ++i) {
            int from = solution.truck_route[i];
            int to = solution.truck_route[i + 1];
            if (from >= 0 && from < n && to >= 0 && to < n) {
                res.drone_active_time += graph.truck_time[from][to];
            }
        }
    }

    if (static_cast<int>(solution.drone_assignments.size()) > graph.drone_count) {
        addValidationError(res, "uses more drone assignment buckets than drone_count");
    }
    double max_drone_load = 0.0;
    for (const auto& kv : solution.drone_assignments) {
        int drone_id = kv.first;
        if (drone_id < 0 || drone_id >= graph.drone_count) {
            addValidationError(res, "invalid drone id " + std::to_string(drone_id));
        }
        double load = 0.0;
        for (int node : kv.second) {
            if (node < 0 || node >= n) {
                addValidationError(res, "drone task contains invalid node " + std::to_string(node));
                continue;
            }
            if (node == graph.depot || node == graph.drone_station) {
                addValidationError(res, "depot/station served by drone: " + std::to_string(node));
            }
            if (graph.is_drone_eligible.empty() || node >= static_cast<int>(graph.is_drone_eligible.size()) ||
                !graph.is_drone_eligible[node]) {
                addValidationError(res, "node served by drone but not eligible: " + std::to_string(node));
            }
            served[node]++;
            load += graph.drone_time[graph.drone_station][node] + graph.drone_time[node][graph.drone_station];
        }
        max_drone_load = std::max(max_drone_load, load);
    }

    for (int node = 1; node < n; ++node) {
        if (served[node] == 0) addValidationError(res, "missing node " + std::to_string(node));
        if (served[node] > 1) addValidationError(res, "duplicated node " + std::to_string(node));
    }

    res.drone_complete_time = res.drone_active_time + max_drone_load;
    res.makespan = std::max(res.truck_time, res.drone_complete_time);
    const double eps = 1e-6;
    if (std::fabs(res.truck_time - solution.truck_completion_time) > eps) {
        addValidationError(res, "truck time mismatch recomputed=" + std::to_string(res.truck_time) +
            " reported=" + std::to_string(solution.truck_completion_time));
    }
    if (std::fabs(res.drone_active_time - solution.station_activation_time) > eps) {
        addValidationError(res, "drone activation mismatch recomputed=" + std::to_string(res.drone_active_time) +
            " reported=" + std::to_string(solution.station_activation_time));
    }
    if (std::fabs(res.drone_complete_time - solution.drone_completion_time) > eps) {
        addValidationError(res, "drone completion mismatch recomputed=" + std::to_string(res.drone_complete_time) +
            " reported=" + std::to_string(solution.drone_completion_time));
    }
    if (std::fabs(res.makespan - solution.makespan) > eps) {
        addValidationError(res, "makespan mismatch recomputed=" + std::to_string(res.makespan) +
            " reported=" + std::to_string(solution.makespan));
    }
    return res;
}
    ...
    std::mutex& result_mutex)
{
    ...
    try {
        ...
        FinalValidationResult finalValidation = validateFinalSolutionStrict(graph, solution);

        {
            ...
            result_file << instance_base << ","
                ...
                << csvEscape(droneStr) << ","
                << solution.init_Id << ","
                << (finalValidation.valid ? 1 : 0) << ","
                << csvEscape(finalValidation.errors)
                << "\n";
        }
    }
    ...
    std::mutex& result_mutex)
{
    ...
    try {
        ...
        const std::string effective_depot_position =
            (ext == ".csv") ? depot_pos : depot_position;

        TSPDSGraph graph = (ext == ".csv")
            ? parseOriginCsvFile(filepath, drone_count, 0.5, std::numeric_limits<double>::max())
            : parseTSPLIBFile(filepath, drone_count, 0.5, std::numeric_limits<double>::max(), effective_depot_position);



        cout << "\n==========================================" << endl;
        cout << "Processing FILE: " << filepath << " (depot: " << effective_depot_position << ", drones: " << drone_count << ")" << endl;
        cout << "==========================================" << endl;
        ...
        std::size_t h2 = std::hash<std::string>{}(effective_depot_position);
        ...
        // -------- 查最优对比 --------
        string search_key2 = instance_name + "_" + effective_depot_position + "_" + to_string(drone_count);
        ...
        const std::string effective_depot_position =
            (ext == ".csv") ? depot_pos : depot_position;
        string search_key = instance_name + "_" + effective_depot_position + "_" + to_string(drone_count);
...
int main() {
    ...
    int NUM_REPEATS = getEnvInt("TSPDS_REPEATS", 20);     // 并行重复次数
    // ========== 进程数==========
    int WORKERS = getEnvInt("TSPDS_WORKERS", 20);
    if (WORKERS < 1) WORKERS = 1;

    // 定义无人机数量数组
    vector<int> drone_counts = getEnvIntList("TSPDS_DRONES", { 1,3,5 }); // 1,3,5,7,9
    vector<string> depot_positions = getEnvStringList("TSPDS_DEPOTS", { "L" }); // "1" "C", "L"
    bool isSmallInstance = getEnvBool("TSPDS_SMALL", false); // Murray小实例=true；TSPLIB=false
    ...
    int murray_sample_limit = getEnvInt("TSPDS_MURRAY_SAMPLE_LIMIT", 20); // 0 表示全部；需要抽样时改成 20 等数量
    ...
    if (!isSmallInstance) {
        std::vector<std::string> instance_filters = getEnvStringList("TSPDS_INSTANCE", {});
        auto matchesInstanceFilter = [&](const std::string& path_str) {
            if (instance_filters.empty()) return true;
            for (const auto& f : instance_filters) {
                if (path_str.find(f) != std::string::npos) return true;
            }
            return false;
        };
        ...
        for (const auto& entry : fs::directory_iterator(input_dir)) {
            ...
            if (ext == ".csv" || ext == ".tsp") {
                ...
                if (matchesInstanceFilter(path_str)) {
                    tsp_files.push_back(path_str);
                }
            }
        }
            ...
            for (const std::string& filename : tsp_files) {
                ...
                if (ext == ".csv") {
                    for (int dc : drone_counts) {
                        jobs.push_back(Job{ filename, "", dc, run_id, false });
                    }
                }
                else {
    ...
    for (int w = 0; w < WORKERS; ++w) {
        ...
        if (pid == 0) {
            ...
            wf << "Instance,DepotPosition,DroneCount,RunId,Seed,Nodes,"
                ...
                "TruckRoute,DroneTasks,Init_id,Valid,ValidationErrors\n";
    ...
    for (int w = 0; w < (int)worker_csv_paths.size(); ++w) {
        ...
        while (std::getline(in, line)) {
            ...
            if (cells.size() < 21) continue; // 列数不够就跳过（避免异常行）
            ...
            bool rowValid = (cells.size() < 20 || cells[19] == "1" || cells[19] == "true" || cells[19] == "TRUE");
