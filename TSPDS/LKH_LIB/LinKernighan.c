...
// 在LinKernighan.c的Gain23函数中
#ifdef HAVE_LONG_LONG
    // 整数版本处理
                    if (llabs(Gain % Precision) > 0) {
                        // 调整Gain值使其满足条件
                        Gain = (Gain / Precision) * Precision;
                    }
#else
    // 浮点版本处理
                    const double tolerance = 1e-6;
                    double remainder = fmod(Gain, Precision);

                    if (fabs(remainder) > tolerance) {
                        // 可选：记录警告
                        if (TraceLevel >= 1) {
                            printf("Warning: Gain adjustment: %f -> ", Gain);
                        }

                        // 调整Gain值
                        Gain = round(Gain / Precision) * Precision;
