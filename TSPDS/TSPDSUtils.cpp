...
double TSPDSUtils::calculateMakespan(TSPDSSolution& solution) {
    ...
    for (const auto& assignment : solution.drone_assignments) {
        ...
        for (int node : assignment.second) {
            droneTotalTime += graph.drone_time[graph.drone_station][node] + graph.drone_time[node][graph.drone_station];
        }
...
void TSPDSUtils::optimizeTruckRouteWithLKHIntern(TSPDSSolution& s)
{
    ...
    auto runSegmentLKH = [this](const std::vector<int>& internalNodes,
        const std::string& tag) -> std::vector<int>
        {
            if (internalNodes.size() < 3) {
                return internalNodes;
            }

            int m = static_cast<int>(internalNodes.size());
            const std::string temp_dir = "lkh_temp_intern";
            std::filesystem::create_directories(temp_dir);

            auto ticks = std::chrono::high_resolution_clock::now().time_since_epoch().count();
            std::ostringstream oss;
            oss << "seg_" << tag << "_" << ticks << "_" << std::this_thread::get_id();
            std::string base = oss.str();
            ...
            tsp << "NAME: TRUCK_SEGMENT\n";
            tsp << "TYPE: TSP\n";
            tsp << "COMMENT: Truck segment optimization\n";
            tsp << "DIMENSION: " << m << "\n";
            tsp << "EDGE_WEIGHT_TYPE: EXPLICIT\n";
            tsp << "EDGE_WEIGHT_FORMAT: FULL_MATRIX\n";
            tsp << "EDGE_WEIGHT_SECTION\n";
            ...
            for (int i = 0; i < m; ++i) {
                int ni = internalNodes[i];
                for (int j = 0; j < m; ++j) {
                    int nj = internalNodes[j];
                    double dist = graph.truck_time[ni][nj];
                    tsp << static_cast<int>(dist) << " ";
                }
                tsp << "\n";
            ...
            while (std::getline(tour, line)) {
                ...
                int idx = std::stoi(line);
                if (idx >= 1 && idx <= m) {
                    perm.push_back(internalNodes[idx - 1]);
                }
            }
            tour.close();
            cleanup_files();

            if (static_cast<int>(perm.size()) != m) {
    ...
    std::vector<int> preOpt = runSegmentLKH(preInternal, "pre");
    std::vector<int> postOpt = runSegmentLKH(postInternal, "post");
...
{
    return graph.drone_time[graph.drone_station][node] + graph.drone_time[node][graph.drone_station];
