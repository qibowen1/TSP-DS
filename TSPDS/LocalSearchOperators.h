...
public:
    ...
    struct TargetScore {
        double phi = 0.0;
        double makespan = 0.0;
        double truck_over = 0.0;
        double drone_over = 0.0;
        double total_over = 0.0;
        double maxjob_over = 0.0;
    };

    TargetScore calculateTargetScore(const TSPDSSolution& solution, double target) const;
    bool acceptTargetMove(const TSPDSSolution& curr, const TSPDSSolution& cand, double target) const;
    TSPDSSolution targetLocalSearch(const TSPDSSolution& start, double target);
    ...
    // 节点交换操作
    TSPDSSolution compoundTruckDroneExchange(TSPDSSolution& solution);
